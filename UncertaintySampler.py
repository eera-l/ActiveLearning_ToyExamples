...
class Sampler(object):
    ...
    def plot_initial_data(self, X, y):
        fig = plt.figure(figsize=(6, 4))
        ax = fig.add_subplot(111)
        ax.scatter(X[:, 0], X[:, 1], c=y)
        ax.set_title('Toy dataset clusters')
        plt.show()
...
def plot_learning_curve(sample_size, classifier, cluster_type, informativeness_measure,
                        train_size_ratio, first_samples_ratio=2, n_attempts=100,
                        noise=0.0, centers=None, clusters_std=None):
    """
    ...
    for i in range(1, n_attempts):
        ...
        sampler = Sampler(classifier=classifier, cluster_type=cluster_type, informativeness_measure=informativeness_measure,
                          random_state=i, train_size=train_size, noise=noise,
                          centers=centers, clusters_std=clusters_std, first_samples_size=int(train_size / first_samples_ratio),
